...
final class StringsStrumsView: UIView {
    ...
    /// 琴弦区域上下留白（统一常量，改这一处即可全局生效）
    private let stringTopMargin: CGFloat = 235
    private let stringBottomMargin: CGFloat = 235
    ...
    private func stringY(_ index: Int) -> CGFloat {
        let usableHeight = bounds.height - stringTopMargin - stringBottomMargin
        let spacing = usableHeight / CGFloat(GuitarConstants.stringCount - 1)
        return stringTopMargin + CGFloat(index) * spacing
    }
    ...
    private func nearestString(for y: CGFloat) -> Int {
        let usableHeight = bounds.height - stringTopMargin - stringBottomMargin
        let spacing = usableHeight / CGFloat(GuitarConstants.stringCount - 1)
        var index = Int(round((y - stringTopMargin) / spacing))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            if pos.y < stringTopMargin - 60 || pos.y > bounds.height - stringBottomMargin + 40 {
